...
# Attempt to read first table via Camelot and fix headers
first_table = None
try:
...
# Extract all tables and build data structures
tables_info = []
grand_total = None
...
with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    ...
    for pi, page in enumerate(pdf.pages):
        ...
        desc_links = {}
        if source == "plumber" and bbox:
            for r, row_obj in enumerate(page.find_tables()[0].rows):
                if r == 0: continue
        ...
        new_hdr = ["Strategy", "Description"] + [h for i, h in enumerate(hdr) if i != desc_i and h.strip()]
        rows_data = []
        row_links = []
        table_total = None
        for ridx, row in enumerate(data[1:], start=1):
            ...
            rest = [cells[i] for i, h in enumerate(hdr) if i != desc_i and h.strip() and i < len(cells)]
...
r.font.name = DEFAULT_SERIF_FONT; r.font.size = Pt(18); r.bold = True
