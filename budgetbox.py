...
try:
    ...
    DEFAULT_SERIF_FONT="DMSerif"; DEFAULT_SANS_FONT="Barlow"
except:
    DEFAULT_SERIF_FONT="Times New Roman"; DEFAULT_SANS_FONT="Arial"
...
try:
    ...
    if tables:
        ...
        if len(raw)>1:
            header0=raw[0]; header1=raw[1]
            if header0[0].lower()=="strategy" and header1[0].lower()=="description":
                first_hdr=[header0[0]]+header1
                first_rows=raw[2:]
                first_table=[first_hdr]+first_rows
            else:
                first_table=raw
except:
    first_table=None
...
with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    ...
    for pi,page in enumerate(pdf.pages):
        ...
        if pi==0 and first_table:
            source="camelot"; data=first_table
        else:
            source="plumber"
        if source=="camelot":
        ...
        for tbl_obj,data,bbox in tables_found:
            ...
            keep_idx=[i for i,h in enumerate(new_hdr) if h and h.lower()!="none"]
            new_hdr=[new_hdr[i] for i in keep_idx]
            ...
            for ridx,row in enumerate(data[1:],start=1):
                ...
                rest=[cells[i] for i in range(len(hdr)) if i!=desc_i]
                full=[strat,desc]+rest
                full=[full[i] for i in keep_idx]
                rows_data.append(full)
                if source=="camelot":
...
for hdr,rows,links,tot in tables_info:
    n=len(hdr); desc_idx=hdr.index("Description") if "Description" in hdr else 1
    ...
    if tot:
        ...
        row=[Paragraph(lbl,bls)] + [Paragraph("",bs)]*(n-2) + [Paragraph(val,brs)]
        wrapped.append(row)
