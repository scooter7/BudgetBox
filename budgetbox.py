...
def reconstruct_table_from_words(table_obj, page_height, x_tolerance=3, y_tolerance=3):
    ...
    if words:
        ...
        for i in range(1, len(words)):
            ...
            if words[i]['top'] > current_line_bottom + (line_height_guess * 0.5): row_lines.append((current_line_top, current_line_bottom)); current_line_top = words[i]['top']; current_line_bottom = words[i]['bottom']
    ...
    if header_words:
        ...
        for i in range(1, len(header_words)):
            ...
            if space_guess > 5: col_boundaries.append((current_col_start, current_col_end)); current_col_start = header_words[i]['x0']; current_col_end = header_words[i]['x1']
    ...
    for word in words:
        ...
        for idx, (c_start, c_end) in enumerate(col_boundaries):
            if max(word['x0'], c_start) < min(word['x1'], c_end): col_idx = idx; break
            elif word_mid_x >= c_start - x_tolerance and word_mid_x <= c_end + x_tolerance: col_idx = idx; break
        if col_idx != -1:
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        ...
        for pi, page in enumerate(pdf.pages):
            ...
            for tbl_idx, tbl in enumerate(page_tables):
                ...
                if data is None or len(data) < 2 or not any(data[0]): data = tbl.extract(x_tolerance=3, y_tolerance=3)
                ...
                if original_desc_idx == -1:
                     common_desc_headers = ["details", "summary", "notes", "content"]; found_common = False
                     for i, h in enumerate(original_hdr):
                          if h:
...
except Exception as e: st.error(f"Error processing PDF: {e}"); import traceback; st.error(traceback.format_exc()); st.stop()
...
for table_index, (hdr, rows_data, row_links_uri_list, table_total_info) in enumerate(tables_info):
    ...
    if not col_widths and num_cols > 0: col_widths = [total_page_width / num_cols] * num_cols # Assign fallback if needed
    if not col_widths: continue # Skip if still no widths
    # --- Build Table Content ---
    ...
    if table_total_info:
        ...
        if '$' not in value: value = next((val.strip() for val in reversed(original_total_row) if val and '$' in str(val)), value)
             ...
             if total_match:
                  label_parsed, value_parsed = total_match.groups()
                  label = label_parsed.strip() if label_parsed and label_parsed.strip() else "Total"
                  value = value_parsed.strip() if value_parsed else ""
             else:
...
# --- Add Grand Total Row (FIXED SYNTAX HERE) ---
if grand_total and tables_info:
    last_hdr, _, _, _ = tables_info[-1]
    num_cols = len(last_hdr) # Semicolon removed
    if num_cols > 0:
        gt_col_widths = [] # Initialize
        desc_actual_idx_in_hdr = -1 # Initialize

        # Try to find 'Description' and calculate widths based on it
        try:
            desc_actual_idx_in_hdr = last_hdr.index("Description")
            desc_col_width = total_page_width * 0.45
            other_cols_count = num_cols - 1

            # This calculation only makes sense if more than 1 column
            if other_cols_count > 0:
                 other_total_width = total_page_width - desc_col_width
                 strategy_idx = -1
            ...
            # If other_cols_count is 0 (meaning num_cols was 1), gt_col_widths remains empty here
            # We handle num_cols == 1 case after the except block

        except ValueError: # 'Description' not found
             desc_actual_idx_in_hdr = -1 # Ensure it's marked as not found
             # Fallback: Equal widths if Description wasn't found (and num_cols > 0)
             gt_col_widths = [total_page_width / num_cols] * num_cols
             # No need for else here, outer if handles num_cols == 0

        # Handle cases based on num_cols *after* trying to find Description
        if num_cols == 1:
             gt_col_widths = [total_page_width]
        elif not gt_col_widths and num_cols > 0: # If try/except failed to set widths for num_cols > 1
            gt_col_widths = [total_page_width / num_cols] * num_cols # Fallback equal widths

        # Build GT table only if widths were determined
        if gt_col_widths:
...
for table_index, (hdr, rows_data, row_links_uri_list, table_total_info) in enumerate(tables_info):
    n = len(hdr);
    if n == 0: continue
    # Width calculation (same fixed logic as PDF)
    ...
    try:
        ...
        # else case n=0 skipped, n=1 handled
    except ValueError:
        ...
        try:
            ...
            if gt_other_count > 0:
                 ...
                 if gt_strat_idx != -1:
                     ...
                     gt_col_widths = [max(0.1*inch, gt_other_w) if i != gt_desc_idx and i != gt_strat_idx else (gt_desc_w if i == gt_desc_idx else gt_strat_w) for i in range(num_cols)] # Use Inches here for PDF
                 else:
                     gt_other_w = gt_other_total_w / gt_other_count
                     gt_col_widths = [gt_other_w if i != gt_desc_idx else gt_desc_w for i in range(num_cols)]
            # Case num_cols = 1 handled after except

        except ValueError:
            gt_desc_idx = -1
            # Fallback equal widths
            gt_col_widths = [TOTAL_W_INCHES / num_cols] * num_cols # Use Inches here for Word width

        # Handle cases after try/except
        if num_cols == 1:
             gt_col_widths = [TOTAL_W_INCHES] # Word width
        elif not gt_col_widths and num_cols > 0: # Fallback if try/except didn't set it
            gt_col_widths = [TOTAL_W_INCHES / num_cols] * num_cols # Word width

        # Create GT table if widths determined
        if gt_col_widths: # Check if list is not empty
             ...
             tblgPr.append(tblgW);
             # Set column widths for GT table using *Inches* for Word
             for idx, col in enumerate(tblg.columns):
                 width_val_gt = 0
                 # Need to recalculate widths in Inches based on indices
                 if idx == gt_desc_idx: width_val_gt = 0.45 * TOTAL_W_INCHES
                 elif gt_strat_idx != -1 and idx == gt_strat_idx: width_val_gt = 0.15 * TOTAL_W_INCHES
                 else: # Calculate 'other' width in inches
                     if gt_strat_idx != -1: # Strategy exists
                         other_total_w_in = TOTAL_W_INCHES * (1 - 0.45 - 0.15)
                         other_cols_count_gt = n - 2
                         width_val_gt = other_total_w_in / other_cols_count_gt if other_cols_count_gt > 0 else TOTAL_W_INCHES / n
                     elif gt_desc_idx != -1 : # Only Description exists
                         other_total_w_in = TOTAL_W_INCHES * (1 - 0.45)
                         other_cols_count_gt = n - 1
                         width_val_gt = other_total_w_in / other_cols_count_gt if other_cols_count_gt > 0 else TOTAL_W_INCHES / n
                     else: # No Desc or Strat found (equal width)
                         width_val_gt = TOTAL_W_INCHES / n
