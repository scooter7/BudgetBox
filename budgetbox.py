...
# ─── 1) Capture all link annotations from the source PDF ───────────────────────
...
# ─── 2) Extract tables, descriptions, and per-row links with pdfplumber ───────
...
with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    ...
    for pi, page in enumerate(pdf.pages):
        ...
        for tbl in page.find_tables():
            ...
            # map each annotation rect to a table row
...
# ─── 3) Build deliverable PDF with ReportLab (no links embedded) ──────────────
pdf_buf = io.BytesIO()
...
# ─── 4) Overlay the real link annotations via PyMuPDF ─────────────────────────
mupdf = fitz.open(stream=pdf_buf.read(), filetype="pdf")
for page_no, annots in enumerate(page_annotations):
    # for each annotation from original
    for rect, uri in annots:
        mupdf[page_no].insert_link({
            "kind": fitz.LINK_URI,
            "from": rect,
            "uri": uri
        })
...
# ─── 5) Build Word deliverable with robust hyperlink recipe ────────────────────
...
for hdr, rows, row_links, tbl_total in tables_info:
    ...
    for i,col_name in enumerate(hdr):
        cell = tblW.rows[0].cells[i]; tcPr = cell._tc.get_or_add_tcPr()
        shd = OxmlElement('w:shd'); shd.set(qn('w:fill'),'F2F2F2'); tcPr.append(shd)
        p = cell.paragraphs[0]; p.text=""
        run = p.add_run(col_name); run.font.name,run.font.size,run.bold = "DMSerif",Pt(10),True
    ...
    for ridx,row in enumerate(rows):
        ...
        for cidx,val in enumerate(row):
            ...
            if cidx==1 and row_links[ridx]:
                ...
                run = p.add_run(str(val)); run.font.name, run.font.size = "Barlow", Pt(9)
    # table total
    if tbl_total:
        label, amt = re.split(r'\$\s*', tbl_total,1); amt="$"+amt.strip()
        rc = tblW.add_row().cells
        for i,tv in enumerate([label]+[""]*(n-2)+[amt]):
            cell = rc[i]; tcPr = cell._tc.get_or_add_tcPr()
            shd = OxmlElement('w:shd'); shd.set(qn('w:fill'),'F2F2F2'); tcPr.append(shd)
            p = cell.paragraphs[0]; p.text=""
            run = p.add_run(tv); run.font.name,run.font.size,run.bold = "DMSerif",Pt(10),True
...
if grand_total:
    ...
    for idx,tv in enumerate(["Grand Total"]+[""]*(n-2)+[grand_total]):
        cell = tblG.rows[0].cells[idx]; tcPr = cell._tc.get_or_add_tcPr()
        shd = OxmlElement('w:shd'); shd.set(qn('w:fill'),'F2F2F2'); tcPr.append(shd)
        p = cell.paragraphs[0]; p.text=""
        run = p.add_run(tv); run.font.name,run.font.size,run.bold = "DMSerif",Pt(10),True
