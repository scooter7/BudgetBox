    ...
    for pi, page in enumerate(pdf.pages):
        if pi == 0 and first_table:
            ...
            for tbl in page.find_tables():
                ...
                tables_found.append((tbl, data, tbl.bbox))
    ...
    for i, row in enumerate(rows):
        ...
        for j, cell in enumerate(row):
            ...
            if j==desc_idx and links_list[i]:
                line.append(Paragraph(f"{txt} <link href='{html.escape(links_list[i])}' color='blue'>- link</link>", bs))
            else:
    ...
    for ridx, row in enumerate(rows):
        ...
        for cidx, val in enumerate(row):
            ...
            if cidx==desc_idx and ridx < len(links_list) and links_list[ridx]:
                p.add_run(" ")
                add_hyperlink(p, links_list[ridx], "- link", font_name=DEFAULT_SANS_FONT, font_size=9)
