...
# Utility: split title/description
...
# Attempt Camelot for first table
first_table = None
try:
...
# Parse PDF to extract tables and totals
tables_info = []
grand_total = None
            ...
            for ridx,row in enumerate(data[1:], start=1):
                ...
                for i,cv in enumerate(cells):
                    ...
                    if re.search(r'\d{1,2}[/-]\d{1,2}[/-]\d{2,4}', v):
                        if not std[2]: std[2] = v
                        elif not std[3]: std[3] = v
                        continue
                    # term expression
                ...
                # SWAP & COMPUTE
                # Move item->monthly if monthly empty
        ...
        for j,cell in enumerate(row):
            ...
            if j==desc_idx and i<len(links) and links[i]:
                line.append(Paragraph(
                    f"{txt} <link href='{html.escape(links[i])}' color='blue'>- link</link>", bs))
            else:
