...
def add_hyperlink(paragraph, url, text, font_name=None, font_size=None, bold=None):
    ...
    if "Hyperlink" not in styles:
        sty = styles.add_style("Hyperlink", WD_STYLE_TYPE.CHARACTER, True)
        sty.font.color.rgb = RGBColor(0x05,0x63,0xC1); sty.font.underline=True
        sty.priority=9; sty.unhide_when_used=True
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        ...
        for pi, page in enumerate(pdf.pages):
            ...
            for tbl in page.find_tables(table_settings=table_settings):
                ...
                # skip spurious header-only tables
                if len(hdr)<4 or not any("Start Date" in h for h in hdr):
                    continue
                ...
                if rows_data:
                    tables.append((new_hdr, rows_data, uris, table_total))
...
for hdr, rows, uris, tot in tables_info:
    ...
    col_ws = [desc_w if i==desc_i else other_w for i in range(n)]
...
try:
    ...
    st.error(f"PDF build error: {e}"); pdf_buf=None
...
sec.left_margin,sec.right_margin,sec.top_margin,sec.bottom_margin = Inches(0.5),Inches(0.5),Inches(0.5),Inches(0.5)

if 'logo' in locals():
    try:
        p=docx_doc.add_paragraph(); r=p.add_run()
        img=Image.open(io.BytesIO(logo)); ratio=img.height/img.width; w_in=5
        r.add_picture(io.BytesIO(logo), width=Inches(w_in)); p.alignment=WD_TABLE_ALIGNMENT.CENTER
    except: pass
