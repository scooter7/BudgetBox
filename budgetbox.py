...
def split_strategy(desc: str):
    """Split top line (strategy) vs rest of description."""
...
def add_hyperlink(p, url, text, font_name=None, font_size=None, bold=None):
    """Append a single run hyperlink “- link” to a python-docx paragraph."""
    part = p.part
    ...
    if "Hyperlink" not in styles:
        ...
        stl.font.color.rgb = RGBColor(0x00,0x33,0x99); stl.font.underline=True
...
def extract_all():
    # open with PyMuPDF for link rects
    mz = fitz.open(stream=pdf_bytes, filetype="pdf")
    linkmaps = []
    for pg in mz:
        ann = pg.annots() or []
        linkmaps.append([
            (a.rect, a.uri) for a in ann if a.type[0]==1 and a.uri
        ])
    # open via pdfplumber
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        texts = [pg.extract_text(x_tolerance=1,y_tolerance=1) or "" for pg in pdf.pages]
        # title from first page
        first = texts[0].splitlines()
        title = next((l for l in first if "proposal" in l.lower()), first[0] if first else "Untitled")
        tables = []
        used_totals = set()
        def find_total(pi):
        ...
        for pi,p in enumerate(pdf.pages):
            # extract_tables yields lists of rows
            for tbl in p.extract_tables():
                ...
                if di is None:
                    di = next((i for i,h in enumerate(hdr) if len(h)>10), 1)
                new_hdr = ["Strategy","Description"] + [h for i,h in enumerate(hdr) if i!=di]
                rows, uris, tot = [], [], None
                # map link rows by mid‐point Y
                x0,y0,x1,y1 = p.within_bbox(p.bbox).extract_table_bbox(tbl)
                n = len(tbl); band=(y1-y0)/n
                lm = linkmaps[pi]
                rowlink = {}
                for rct, uri in lm:
                    mid = (rct.y0+rct.y1)/2
                    if y0<mid<y1:
                        rid = int((mid-y0)//band)-1
                        if 0<=rid<len(tbl)-1:
                            rowlink[rid]=uri
...
try:
    p=docx.add_paragraph(); r=p.add_run()
    img = Image.open(io.BytesIO(logo)); ratio=img.height/img.width; w=Inches(5)
    r.add_picture(io.BytesIO(logo), width=w); p.alignment=WD_TABLE_ALIGNMENT.CENTER
except: pass

p=docx.add_paragraph(); p.alignment=WD_TABLE_ALIGNMENT.CENTER
r=p.add_run(proposal_title); r.font.name=SERIF; r.font.size=Pt(18); r.bold=True
docx.add_paragraph()
...
for hdr, rows, uris, tot in tables_info:
    n=len(hdr); di=hdr.index("Description")
    dw=0.45*TW; ow=(TW-dw)/(n-1)
    tbl=docx.add_table(rows=1,cols=n,style="Table Grid"); tbl.alignment=WD_TABLE_ALIGNMENT.CENTER
    ...
    for ridx,row in enumerate(rows):
        ...
        for ci,cell in enumerate(row):
            c=rc[ci]; p=c.paragraphs[0]; p.text=""
...
# save & download
pdf_buf = pdf_buf.getvalue() if (pdf_buf := pdf_buf.getvalue() or pdf_buf) else None
docx_buf = io.BytesIO()
try:
    docx.save(docx_buf)
...
c1,c2 = st.columns(2)
if pdf_buf:
    c1.download_button("📥 Download PDF", pdf_buf, "deliverable.pdf", "application/pdf")
else:
    c1.error("PDF failed")
if docx_buf:
    c2.download_button("📥 Download DOCX", docx_buf, "deliverable.docx",
                       "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
else:
    c2.error("DOCX failed")
