...
# ─── Extract First Table with Camelot & Fix Header ─────────────────────────────
first_table = None
try:
    cams = camelot.read_pdf(
    ...
    if cams and len(cams) > 0:
        df = cams[0].df
        # If top row is mostly blank/"None", promote second row to header
...
# ─── Extract All Tables via pdfplumber + Combine ────────────────────────────────
tables_info = []
grand_total = None
proposal_title = "Untitled Proposal"

with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    # capture page texts for title + totals
    ...
    for pi,page in enumerate(pdf.pages):
        ...
        if source == "plumber":
            # keep only the columns we want, drop any empty/None ones
        ...
        if source == "plumber":
            # build link map for description cells
            desc_links = {}
