...
def split_by_bold_fitz(doc, page_number, bbox):
    x0, y0, x1, y1 = bbox
    ...
    lines = {}
    for span in spans:
        key = round(span["bbox"][1], 1)
        lines.setdefault(key, []).append(span)
    strategy_lines = []
    normal_lines = []
    for key in sorted(lines):
        text = "".join(s["text"] for s in lines[key])
        if any(s["flags"] & 2 for s in lines[key]):
            strategy_lines.append(text)
        else:
            normal_lines.append(text)
    strat = " ".join(strategy_lines[:2]).strip()
    desc = " ".join(strategy_lines[2:] + normal_lines).strip()
    return strat, desc
...
with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    ...
    pot = next((ln.strip() for ln in first_lines if "proposal" in ln.lower() and len(ln.strip())>5), None)
    if pot:
        proposal_title = pot
    elif first_lines:
    ...
    for pi, page in enumerate(pdf.pages):
        ...
        for tbl_obj, data, bbox in tables_found:
            ...
            for i,h in enumerate(hdr):
                if not h or h.lower()=="none": continue
                hl = h.lower()
                if any(x in hl for x in ["strategy","service","product"]): header_mapping[i]=0
                elif any(x in hl for x in ["description","details"]): header_mapping[i]=1
                elif any(x in hl for x in ["start date","start","begin"]): header_mapping[i]=2
                elif any(x in hl for x in ["end date","end","finish"]): header_mapping[i]=3
                elif any(x in hl for x in ["term","duration","period","months"]): header_mapping[i]=4
                elif any(x in hl for x in ["monthly","per month","rate","recurring"]): header_mapping[i]=5
                elif any(x in hl for x in ["item total","subtotal","line total","amount"]): header_mapping[i]=6
                elif any(x in hl for x in ["note","comment","additional"]): header_mapping[i]=7
            rows_data = []
            row_links = []
            table_total = None
            for ridx,row in enumerate(data[1:], start=1):
                ...
                if ("total" in fc or "subtotal" in fc) and any("$" in c for c in cells):
                    if table_total is None:
                        table_total = cells
                    continue
                raw_text = cells[desc_i] if desc_i < len(cells) else ""
                if tbl_obj != "camelot" and bbox:
                    strat, desc = split_by_bold_fitz(doc_fitz, pi, bbox)
                    if not strat:
                        strat, desc = split_cell_text(raw_text)
                else:
                    strat, desc = split_cell_text(raw_text)
                std = [""] * len(STANDARD_HEADERS)
                std[0], std[1] = strat, desc
                for i,cv in enumerate(cells):
                    if i==desc_i or not cv.strip(): continue
                    v = cv.strip()
                    ...
                    if re.fullmatch(r"\d{1,3}", v):
                        ...
                        continue
                    if "$" in v:
                        if not std[5] and any(x in v.lower() for x in ["month","mo"]):
                            std[5] = v
                        elif not std[6]:
                            std[6] = v
                        continue
                ...
                if not any(std[2:]):
                    ...
                    for cv in rem:
                        ...
                        if not std[7]:
                            std[7] = v
                if not std[5] and std[6]:
                    std[5] = std[6]
                    std[6] = ""
                if std[5] and std[4] and not std[6]:
                    amt = float(std[5].replace("$","").replace(",",""))
                    m = re.search(r"\d+", std[4])
                    if m:
                        months = int(m.group())
                        std[6] = f"${amt*months:,.2f}"
                rows_data.append(std)
...
brs = ParagraphStyle("BR", fontName=DEFAULT_SERIF_FONT, fontSize=10, alignment=TA_LEFT, spaceBefore=6)
elements = []
