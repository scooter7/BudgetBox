...
def is_data_row(cells):
    text = " ".join(cells)
    if re.search(r'\$\s*\d', text): return True
    if re.search(r'\d{1,2}[/-]\d{1,2}[/-]\d{2,4}', text): return True
    if re.search(r'\b\d+\s*(?:month|mo|yr|year)\b', text.lower()): return True
    return False
...
with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    ...
    for pi, page in enumerate(pdf.pages):
        if pi==0 and first_table:
            ...
            found = [(tbl, tbl.extract(x_tolerance=1, y_tolerance=1), tbl.bbox, tbl.rows)
...
for hdr, rows, links, tot in tables_info:
    ...
    if tot:
        ...
        wrapped.append([Paragraph(lbl, bs)] + [Paragraph("", bs)]*(len(hdr)-2) + [Paragraph(val, bs)])
